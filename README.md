...
Modèle cible : **Ministral-3B** (Mistral AI).
...
1. **Collecte d'activations** — faire passer un jeu d'instructions nuisibles et bénignes dans le modèle, capturer le residual stream à chaque couche.
2. **Direction de refus** — différence de moyennes (nuisible − bénin) par couche, normalisée.
3. **Ablation** — orthogonalisation des poids d'écriture (attention, MLP) par rapport à la direction sélectionnée.
...
- [ ] Phase 1 — Reproduction de la baseline (direction de refus, orthogonalisation) sur Ministral-3B
...
Ce projet est une contribution de recherche en interprétabilité et sécurité des LLM, pas un outil prêt à l'emploi pour contourner des garde-fous en production. Les jeux d'instructions « nuisibles » utilisés proviennent de benchmarks publics existants (ex. AdvBench, HarmBench) et servent uniquement à caractériser la direction de refus — aucun contenu nuisible généré n'est publié dans ce dépôt. Les résultats visent à documenter la fragilité du safety fine-tuning, dans la continuité des travaux d'Arditi et al. et de Labonne.
